...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("showBottomButton").checked =
    settings.showBottomButton;
  document.getElementById("showProgressRing").checked =
    settings.showProgressRing;
  document.getElementById("showTooltips").checked = settings.showTooltips;
  document.getElementById("customScrollEnabled").checked =
    settings.customScrollEnabled;
  document.getElementById("topScrollPosition").value =
    settings.topScrollPosition;
  document.getElementById("bottomScrollPosition").value =
    settings.bottomScrollPosition;
  ...
  document.getElementById("autoHideSettings").style.display = settings.autoHide
    ? "block"
    : "none";
...
async function saveSettings() {
  try {
    const position = document.querySelector(".position-option.active").dataset
      .position;
    ...
    const settings = {
      ...
      customScrollEnabled: document.getElementById("customScrollEnabled")
        .checked,
      topScrollPosition:
        parseInt(document.getElementById("topScrollPosition").value) || 0,
      bottomScrollPosition:
        parseInt(document.getElementById("bottomScrollPosition").value) || 100,
    ...
    if (settings.customScrollEnabled) {
      if (settings.scrollType === "percentage") {
        if (
          settings.topScrollPosition < 0 ||
          settings.topScrollPosition > 100
        ) {
          showStatus("Top scroll position must be between 0 and 100%", "error");
          return;
        }
        if (
          settings.bottomScrollPosition < 0 ||
          settings.bottomScrollPosition > 100
        ) {
          showStatus(
            "Bottom scroll position must be between 0 and 100%",
            "error",
          );
          return;
        }
